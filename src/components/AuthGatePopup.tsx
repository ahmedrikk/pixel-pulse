...
export function AuthGatePopup() {
  ...
  useEffect(() => {
    if (!isAuthModalOpen || mode !== "main" || !GOOGLE_CLIENT_ID || !googleButtonRef.current) return;
    ...
    const renderGoogleButton = async () => {
      ...
      window.google.accounts.id.renderButton(googleButtonRef.current, {
        ...
        shape: "pill",
        logo_alignment: "left",
        width: Math.min(360, googleButtonRef.current.clientWidth || 356),
      });
    };

    const existing = document.querySelector<HTMLScriptElement>('script[data-talus-google-identity]');
    if (window.google) {
      ...
      script.async = true;
      script.defer = true;
      script.dataset.talusGoogleIdentity = "true";
  ...
  }, [isAuthModalOpen, mode, closeAuthModal]);
              ...
              {mode === "main" ? (
                <div className="flex flex-col items-center text-center pt-4">
                  ...
                  <div className="w-full flex flex-col gap-3 mt-6">
                    {/* Google */}
                    {isLoading === "google" ? (
                      <div className="flex h-11 w-full items-center justify-center rounded-xl border border-border bg-card">
                        <Loader2 className="h-4 w-4 animate-spin text-foreground" />
                      </div>
                    ) : GOOGLE_CLIENT_ID ? (
                      <div ref={googleButtonRef} className="flex h-11 w-full items-center justify-center overflow-hidden rounded-full" />
                    ) : (
                      <button
