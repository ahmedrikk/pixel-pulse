...
function SectionHeader({ icon, title, context, action }: { icon: React.ReactNode; title: string; context: React.ReactNode; action?: React.ReactNode }) {
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-border/50 bg-gradient-to-r from-accent/10 to-transparent px-4 py-3.5 sm:px-5">
      ...
      <span className="rounded-full bg-secondary px-2.5 py-1 text-tiny-label font-medium text-muted-foreground">{context}</span>
    ...
    <div className="grid grid-cols-[1fr_auto_1fr] items-stretch gap-2 p-4 sm:gap-4 sm:p-5">
      {cards.map((item, index) => (
        <div key={`${item.name}-${index}`} className="contents">
          {index === 1 && <div className="flex items-center text-tiny-label font-black text-muted-foreground">VS</div>}
          <div className={`min-w-0 rounded-xl border p-3 text-center transition-colors sm:p-4 ${item.active ? "border-[1.5px] border-primary bg-primary/10" : "border-border bg-card"}`}>
            ...
            <p className="mt-1 text-tiny-label text-muted-foreground">{round.categoryLabel}</p>
    ...
    <section className="border-b border-border bg-card" aria-labelledby="higher-lower-title">
      ...
      {isLoading ? <div className="h-64 animate-pulse bg-secondary" /> : error || !round ? <EmptyState text="Higher or Lower is warming up." /> : (
        <>
          ...
          <div className="flex items-center justify-between border-t border-border px-5 py-3 text-tiny-label text-muted-foreground">
...
function SentimentBars({ question }: { question: SentimentQuestion }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between pt-1 text-tiny-label text-muted-foreground"><span>{total.toLocaleString()} votes today</span>{question.gameTag && <span className="rounded-full bg-secondary px-2 py-1 text-primary">🔥 {question.gameTag}</span>}</div>
    </div>
  );
}
...
function SentimentCard({ question }: { question: SentimentQuestion }) {
  ...
  return (
    <article className="rounded-xl border border-border bg-card p-4 transition-colors hover:bg-secondary/20">
      ...
      {question.userVote && <div className="mt-3 flex items-center gap-2"><span className="inline-flex rounded-full bg-green-500/10 px-2 py-1 text-tiny-label font-semibold text-green-600 dark:text-green-400">You voted {question.userVote === "yes" ? "👍" : "👎"}</span><button disabled={isVoting} onClick={() => submit(question.userVote === "yes" ? "no" : "yes")} className="text-tiny-label font-semibold text-primary hover:underline">Switch to {question.userVote === "yes" ? "👎" : "👍"}</button></div>}
    </article>
  );
}
        ...
        {isLoading ? <div className="h-48 animate-pulse bg-white/5" /> : !fact ? <div className="p-8 text-center text-sm text-white/50">No curated moment is published for {monthDay} yet.</div> : (
          <div className="p-4 sm:p-5">
            ...
            <div className="grid grid-cols-[76px_1fr] gap-4">
              <div className="flex h-20 flex-col items-center justify-center rounded-xl border border-white/10 bg-white/5"><b className="text-2xl text-[#FCD34D]">{fact.year}</b><span className="text-tiny-label text-white/40">{new Date().getFullYear() - fact.year} yrs ago</span></div>
              <div><h3 className="text-base font-bold">{fact.headline}</h3><p className="mt-2 text-xs leading-relaxed text-white/55">{fact.description}</p>{fact.gameTag && <span className="mt-3 inline-flex rounded-full bg-white/10 px-2 py-1 text-tiny-label text-white/70">{fact.gameTag}</span>}</div>
            </div>
            <div className="mt-5 flex items-center"><button type="button" onClick={() => void shareFact()} className="inline-flex items-center gap-2 rounded-lg bg-white/10 px-3 py-2 text-tiny-label font-semibold hover:bg-white/15"><Share2 className="h-3 w-3" />Share this moment</button><div className="ml-auto flex items-center gap-1"><button aria-label="Previous history fact" onClick={() => setIndex((index - 1 + facts.length) % facts.length)} className="rounded-lg p-2 hover:bg-white/10"><ChevronLeft className="h-4 w-4" /></button><span className="text-tiny-label text-white/35">{index + 1}/{facts.length}</span><button aria-label="Next history fact" onClick={() => setIndex((index + 1) % facts.length)} className="rounded-lg p-2 hover:bg-white/10"><ChevronRight className="h-4 w-4" /></button></div></div>
          </div>
        )}
      </div>
    </section>
...
export function FeedHigherLowerCard() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="flex items-center justify-between border-t border-border px-4 py-3 text-tiny-label text-muted-foreground"><span>👥 {round.totalGuesses.toLocaleString()} guessed today</span>{result && <Link onClick={() => void refetch()} className="font-bold text-primary" to="/hub#higher-lower">See full results on Hub →</Link>}</div>
    </div>
  );
}
