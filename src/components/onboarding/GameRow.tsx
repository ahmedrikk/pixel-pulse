  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-all ${
        selected ? 'bg-blue-50 border border-[#3d59e0]' : 'border border-transparent hover:bg-gray-50'
      }`}
    >
      <img
        src={coverUrl}
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium truncate ${selected ? 'text-[#3d59e0]' : 'text-gray-900'}`}>{name}</p>
        <p className="text-xs text-gray-500">{genre}</p>
      </div>
      <div className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center ${
        selected ? 'bg-[#3d59e0] border-[#3d59e0]' : 'border-gray-300'
      }`}>
